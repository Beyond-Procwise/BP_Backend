...
async def session_status_ws(
    ...
    On connect, immediately checks if the session is already resolved.
    If so, sends the result right away — fixes the race condition where
    processing finishes before the client connects.
    """
