...
def insert_response(row: SupplierResponseRow) -> None:
    ...
    in_reply_to_json = None
    if row.in_reply_to:
        try:
            in_reply_to_json = json.dumps(list(row.in_reply_to))
        except Exception:
            in_reply_to_json = None
    references_json = None
    if row.references:
        try:
            references_json = json.dumps(list(row.references))
        except Exception:
            references_json = None

    currency = getattr(row, "currency", None)
    payment_terms = getattr(row, "payment_terms", None)
    warranty = getattr(row, "warranty", None)
    validity = getattr(row, "validity", None)
    exceptions = getattr(row, "exceptions", None)
    tables_json = None
    ...
    with get_conn() as conn:
        ...
        insert_query = (
            ...
            "response_text, response_body, body_html, response_from, round_number, "
            "in_reply_to, reference_ids, response_date, original_message_id, original_subject, "
            "match_confidence, match_score, price, currency, payment_terms, warranty, validity, exceptions, "
            "lead_time, response_time, received_time, processed, dispatch_id, matched_on, raw_headers) "
            ...
            "body_html=COALESCE(EXCLUDED.body_html, proc.supplier_response.body_html), "
