...
class ProcessMonitorWatcher:
    ...
    def _process_record(self, record: Dict[str, Any]) -> None:
        ...
        try:
            # There is one extraction pipeline: src/services/extraction/
            # (regex-primary L1, engineered L2, judge L3, flat-column _raw).
            #
            # This used to be selected by EXTRACTION_RENOVATION_ENABLED, with a second,
            # older pipeline behind the else branch. A flag that silently swaps the engine
            # is a trap: start the process without .env and it runs the wrong extractor,
