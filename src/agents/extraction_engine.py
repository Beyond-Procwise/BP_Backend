...
AGENT_NICK_MODEL = os.getenv("PROCWISE_EXTRACTION_MODEL", "BeyondProcwise/AgentNick:extract")
...
INVOICE_TEMPLATE = {
    "invoice number": "unique invoice ID e.g. INV-2024-001",
    "purchase order number": "PO reference number",
    "invoice date": "date invoice was issued DD/MM/YYYY",
    "due date": "payment due date",
    "vendor or seller name": "company that sent the invoice (NOT the bill-to company, NOT bank details)",
    "vendor or seller address": "address of the vendor/seller company",
    "bill to or invoice to company name": "company being billed (the buyer)",
    "bill to or invoice to address": "",
    "subtotal amount before tax": "amount before VAT/tax (smaller than total)",
    "tax amount": "VAT/tax monetary amount (always smaller than subtotal)",
    "total amount after tax": "final amount including tax (largest number)",
    "currency code": "3-letter code: GBP USD EUR",
    "bank name": "",
    "bank account number": "",
    "bank sort code": "",
    "phone number": "",
...
PO_TEMPLATE = {
    "purchase order number": "PO ID e.g. PO526809",
    "order date": "date the PO was issued",
    "quote number": "reference quote number if mentioned",
    "buyer or ordering company name": "company placing the order",
    "buyer or ordering company address": "",
    "supplier or recipient name": "company fulfilling the order (the vendor/recipient)",
    "supplier or recipient address": "",
    "subtotal amount before tax": "sum of line items before tax",
    "discount amount": "",
    "tax amount": "VAT/tax amount (smaller than subtotal)",
    "total amount after tax": "final total including tax",
...
def map_line_items_invoice(result: dict[str, Any]) -> list[dict[str, str]]:
    ...
    for i, item in enumerate(items, start=1):
        ...
        mapped.append({
            ...
            "tax_percent": tax_pct,
            "po_id": po_id,
            "line_amount": _round_money(_parse_numeric(line_total)),
            "tax_amount": _round_money(line_tax),
            "total_amount_incl_tax": _round_money(total_with_tax),
            "created_date": "",
            "created_by": "",
            "last_modified_by": "",
            "last_modified_date": "",
...
def map_po_line_items(result: dict[str, Any]) -> list[dict[str, str]]:
    ...
    for i, item in enumerate(items, start=1):
        ...
        mapped.append({
            ...
            "unit_of_measue": "",  # DB column has typo — must match
            "currency": currency,
...
QUOTE_TEMPLATE = {
    "quote number": "unique quote ID e.g. QUT136586",
    "quote date": "date the quote was issued",
    "validity or pay by date": "expiry date of the quote",
    "purchase order number": "linked PO number if any",
    "buyer or billed to company name": "company receiving the quote (the buyer)",
    "buyer or billed to company address": "",
    "supplier or quoting company name": "company providing the quote (NOT the buyer, NOT bank details)",
    "supplier or quoting company address": "",
    "subtotal amount before tax": "sum of line items before tax",
    "discount amount": "",
    "tax amount": "VAT/tax amount (always smaller than subtotal)",
    "total amount after tax": "final total including tax (largest amount)",
    "currency code": "GBP USD EUR",
    "payment terms": "",
    "phone number": "",
    "email address": "",
