...
DOC_TYPE_KEYWORDS = {
    "Invoice": [
        ...
        "amount due", "tax invoice", "bill to", "remittance",
        "payment due", "invoice total",
    ],
    "Purchase_Order": [
        "purchase order", "po number", "po no", "po date",
        "purchase requisition", "order date", "ship to",
        "delivery date", "order total",
    ],
    "Quote": [
        "quotation", "quote number", "quote no", "quote date",
        "estimate", "proposal", "validity", "valid until",
        "quoted price",
    ],
    "Contract": [
        "contract", "agreement", "terms and conditions",
        "effective date", "governing law", "jurisdiction",
        "parties", "whereas", "hereby", "witnesseth",
    ],
}
...
def _initialize_qdrant_collection_idempotent(
    ...
    try:
        ...
        logger.warning("Qdrant init skipped or failed (will handle on upsert).", exc_info=True)
...
class DataExtractionAgent(BaseAgent):
    ...
    def __init__(self, agent_nick):
        ...
        # Initialize ML/DL ensemble extraction pipeline for high-accuracy extraction
        try:
            from agents.ml_extraction_pipeline import MLExtractionPipeline
            embedding_model = getattr(agent_nick, "embedding_model", None)
            self._ml_pipeline = MLExtractionPipeline(
                embedding_model=embedding_model,
                ollama_caller=self.call_ollama if hasattr(self, "call_ollama") else None,
                extraction_model=self.extraction_model,
            )
            logger.info("ML extraction pipeline initialized successfully")
    ...
    def _extract_structured_data(
        ...
        # Phase 1: ML Ensemble Pipeline (primary extraction)
        ml_header: Dict[str, Any] = {}
        ml_report: Dict[str, Any] = {}
        if self._ml_pipeline is not None:
            try:
                ml_result = self._ml_pipeline.extract(
                    text=text,
                    file_bytes=file_bytes,
                    doc_type=doc_type,
                    source_hint=source_hint,
                )
                ml_header = ml_result.get("header", {})
                ml_report = ml_result.get("extraction_report", {})
                logger.info(
                    "ML pipeline: %d fields extracted, confidence=%.2f%%, coverage=%.2f%%",
                    ml_report.get("final_fields", 0),
                    ml_result.get("confidence_score", 0) * 100,
                    ml_result.get("coverage", {}).get("weighted_coverage", 0) * 100,
                )
            except Exception as exc:
                logger.warning("ML pipeline extraction failed, falling back to legacy: %s", exc)

        # Phase 2: Legacy extraction as supplement (fills gaps the ML pipeline missed)
        header = self._parse_header_improved(text, file_bytes, source_hint=source_hint)

        # Phase 3: Merge ML results with legacy results (ML takes priority)
        # ML pipeline results have higher confidence, so they override legacy values
        if ml_header:
            ml_confidence = ml_header.pop("_field_confidence", {})
            ml_context = ml_header.pop("_field_context", {})
            ml_methods = ml_header.pop("_field_methods", {})

            legacy_confidence = header.pop("_field_confidence", {})
            legacy_context = header.pop("_field_context", {})

            # Merge: ML values take priority over legacy when ML has higher confidence
            for field_name, ml_value in ml_header.items():
                if ml_value is None:
                    continue
                ml_conf = ml_confidence.get(field_name, 0.0)
                legacy_conf = legacy_confidence.get(field_name, 0.0)

                if ml_conf >= legacy_conf or not header.get(field_name):
                    header[field_name] = ml_value

            # Merge confidence and context metadata
            merged_confidence = {**legacy_confidence, **ml_confidence}
            merged_context = {**legacy_context, **ml_context}
            header["_field_confidence"] = merged_confidence
            header["_field_context"] = merged_context
            header["_field_methods"] = ml_methods

        logger.info(f"Phase 1+2 complete: {len(header)} header fields extracted")
        ...
        report = {
            "extraction_method": extraction_method or "ml_ensemble",
    ...
    def _keyword_classify_doc_type(self, text: str) -> str:
        ...
        for dtype, kws in DOC_TYPE_KEYWORDS.items():
            ...
            for kw in kws:
                ...
                matches = len(re.findall(pattern, snippet))
        ...
        # Require a minimum score and margin over second-best
        sorted_scores = sorted(scores.values(), reverse=True)
        margin = sorted_scores[0] - sorted_scores[1] if len(sorted_scores) > 1 else sorted_scores[0]
        return best_type if best_score >= 2.0 and margin >= 1.0 else "Other"
