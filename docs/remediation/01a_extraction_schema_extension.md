...
| 10 | `.gitignore`'s line-1 `/docs/` hid 54 tracked files from `git add`, which had already cost a real miss. **Only `docs/remediation/` is negated.** A first attempt also negated `docs/superpowers/` and surfaced ~50 previously-ignored plans and specs; that was reverted. `docs/superpowers/` stays ignored and continues to be managed by explicit `git add -f`. |

**What is still not covered.** All of this remains synthetic-fixture validation (§7, blocker B1). A scoped re-review traced the delivered regexes against adversarial strings and found four residual gaps. **None can cause harm today, because `proc.bp_contracts` holds 0 rows in both databases. All four must close before a contract corpus lands.**

1. **Qualifiers that no field claims.** The guard blocks `parent`, `master`, `principal`, `amends`, `amendment to`, `supplements`, `varies`. It does not block `Original`, `Prior`, `Underlying`, `Head`, `Prime`, or `Framework Agreement No:`. "Original Contract Number: X" is standard amendment drafting, and it still resolves `contract_id` to the master — the full silent-overwrite consequence, with no compensating `parent_contract_id` claim. Fix: add those words to **both** the `contract_id` lookbehind set and `parent_contract_id`'s own anchor, preserving the symmetry.

2. **Whitespace variance defeats a fixed-width lookbehind.** `"Parent  Contract Number:"` (two spaces) and `"Parent\nContract Number:"` both slip the guard, because a lookbehind cannot express `\s+`. docling output routinely carries doubled spaces and line breaks between label tokens. Python cannot express this in a lookbehind at all, so the practical backstop is a document invariant asserting `contract_id != parent_contract_id` — `contract.yaml`'s `invariants: []` on that field is currently empty and is the right home for it.

3. **`term_months` has the same word-list incompleteness.** `Renewal Term` and `Extension Term` are blocked; `Extended Term`, `Further Term` and `Option Term` are not, and each yields the renewal length as the contract term.

4. **"amends **the** Contract No: X"** is not blocked. An earlier draft of this record claimed such phrasings "fall to `parent_contract_id`'s own higher-prior claim" — **that claim was wrong.** The re-review traced it live: both fields resolve to the master's identifier, so `contract_id` is still overwritten.

Two narrow regressions were accepted in exchange for the placeholder rejection, both failing in the safe direction (nothing extracted, rather than something wrong): a label with no connector at all (`"Contract No MSA-2024-0087"`) and a label/value split across two lines now yield nothing; and a legitimate identifier beginning `NA-` or `SEE-` is rejected as a placeholder. Both are one-line fixes when a real corpus shows they matter — appending `\b` to the label alternation, and tightening the placeholder lookahead to `(?![\w\-\/])`.
