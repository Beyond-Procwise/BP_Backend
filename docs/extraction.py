...
class DOCXParser:
    ...
    def _cell_text(cell) -> str:
        """Extract text from a table cell, falling back to XML parsing."""
        text = cell.text.strip()
        if text:
            return text
        # Fallback: read w:t nodes directly from the XML
    ...
    def _extract_body_xml_text(doc) -> list[str]:
        """Extract text from w:t nodes grouped by w:p paragraphs.

        Catches text in text boxes, structured content tags, and other
        elements that python-docx's paragraph/table API doesn't expose.
        Cleans up fragmented w:t runs (e.g., 'Apr 2' + '4' -> 'Apr 24').
        """
        ...
        for p_elem in doc.element.body.iter("{%s}p" % ns["w"]):
            ...
            if not t_nodes:
                continue
            # Concatenate runs, collapsing fragments
            raw = "".join(t.text or "" for t in t_nodes)
            # Clean up fragmented numbers: "Apr 2 4 , 202 3" -> "Apr 24, 2023"
            raw = re.sub(r"(\d)\s+(\d)", r"\1\2", raw)
            raw = re.sub(r"\s+,", ",", raw)
            line = raw.strip()
            if not line:
                continue
            # Split concatenated label:value pairs onto separate lines
            line = re.sub(
    ...
    def extract_text(file_path: str) -> str:
        ...
        # Append key-value fields from XML not captured by paragraphs/tables
        existing_text = "\n".join(parts)
        xml_lines = DOCXParser._extract_body_xml_text(doc)
        for line in xml_lines:
            # Only append lines with label:value patterns not already present
            if ":" in line and line not in existing_text:
                # Check if the value part is already in existing text
...
_PO_NUM_PATTERNS = [
    ...
    re.compile(
        r"(?:p\.?o\.?\s*(?:no|number|#|num|ref(?:erence)?)?)\s*[.:\s|]+((?:PO\s*)?[\w\-/]{3,20})",
        re.IGNORECASE,
    ),
    re.compile(
...
_TOTAL_PATTERNS = [
    re.compile(
        rf"(?:^|\n)\s*TOTAL\s*\([A-Z]{{3}}\)\s*[:\s|]*({_CURR}?\s*[\d,]+\.?\d*)",
        re.IGNORECASE,
    ),
    re.compile(
        rf"(?:^|\n)\s*TOTAL\s*[:\s|]*\n\s*({_CURR}?\s*[\d,]+\.?\d*)",
...
_SUBTOTAL_PATTERNS = [
    ...
    re.compile(
        rf"(?:sub[\s\-]?total)\s*(?:\([A-Z]{{3}}\))?\s*[\s:|]\s*({_CURR}?\s*[\d,]+\.?\d*)",
        re.IGNORECASE,
    ),
]
...
_TAX_PATTERNS = [
    ...
    re.compile(
        rf"(?:tax|vat|gst|sales\s*tax)\s*(?:\([\d.]+%\))?\s*\n\s*({_CURR}?\s*[\d,]+\.?\d*)",
        re.IGNORECASE,
    ),
]
...
def _parse_pipe_delimited_table(text: str) -> list[dict[str, str]]:
    """Parse tables where columns are separated by | (common in DOCX extraction)."""
    lines = text.split("\n")
    # Find a header row containing pipes
    header_idx = None
    for i, line in enumerate(lines):
        ...
        id_keywords = {"item", "item id", "id", "sku", "code", "item no", "item #"}
        has_desc = any(c in desc_keywords for c in cells)
        has_qty = any(c in qty_keywords for c in cells)
        # "item" alone counts as desc if there's no separate description column
    ...
    # Map columns by keyword — first pass: find description and ID
    ...
    for ci, h in enumerate(header_cells):
        ...
        if h_lower in ("description", "item description", "product",
            ...
            if has_separate_desc:
                id_col = ci  # This is an ID column, not description
            else:
                desc_col = ci  # "Item" is the description column
    ...
    for line in lines[header_idx + 1:]:
        ...
        if not desc:
            continue
        # Check if any cell in this row is a stop label
        ...
        # Skip rows where qty is not a number (summary/footer)
...
class PurchaseOrderExtractor:
    ...
    def _extract_buyer_name(self, text, spacy_ents, nuext_ents, layout_fields=None):
        candidates: list[Candidate] = []
        layout_fields = layout_fields or {}
        # Check "BILL TO" block first — highest confidence for PO buyer
        bill_to = find_bill_to_q(text)
        if bill_to:
            name = bill_to.split(",")[0].strip()
            if name and len(name) > 1:
                # Use layout score + boost to ensure bill-to wins over heuristic
        ...
        # Only consider ORGs that appear in or after the BILL TO section
        bill_to_pos = text_lower.find("bill to")
        vendor_pos = text_lower.find("vendor")
        for org in spacy_ents.get("ORG", []):
            if _is_clean_ner(org):
                clean = _clean_value(org)
                pos = text_lower.find(clean.lower())
                # Skip ORGs in the VENDOR section (before BILL TO)
...
def _load_supplier_names():
    ...
    if _SUPPLIER_NAMES:
        return
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "supplier_names.json")
    try:
        with open(path, encoding="utf-8") as f:
            data = json.load(f)
        _SUPPLIER_NAMES = data.get("suppliers", [])
        _SUPPLIER_NAMES_LOWER = {n.lower(): n for n in _SUPPLIER_NAMES}
        _SUPPLIER_NAMES_NORM = {_normalize(n): n for n in _SUPPLIER_NAMES}
        _SUPPLIER_CORE_TOKENS = {n: _core_tokens(n) for n in _SUPPLIER_NAMES}
    except (FileNotFoundError, json.JSONDecodeError):
        _SUPPLIER_NAMES = []
        _SUPPLIER_NAMES_LOWER = {}
        _SUPPLIER_NAMES_NORM = {}
        _SUPPLIER_CORE_TOKENS = {}
...
def map_purchase_order(result: dict[str, Any]) -> dict[str, str]:
    ...
    # Clean trailing labels from delivery address
    if delivery_addr:
...
_QUOTE_DATE_PATTERNS_Q = [
    re.compile(
        r"(?:order\s*date|quote\s*date|quotation\s*date)"
        r"\s*[:\s|]+\n?([\s\S]{0,200})",
        re.IGNORECASE,
    ),
    re.compile(
...
class QuoteExtractor(BaseExtractor):
    ...
    def _extract_line_items(self, text, word_boxes, layout_fields=None, raw_text=""):
        ...
        scored_sets.sort(key=lambda x: x[0], reverse=True)
        best_score, best_items = scored_sets[0]
        # Enrich truncated descriptions from flat text if available
        flat_text = layout_fields.get("_flat_text", "")
    ...
    def _enrich_descriptions(items: list[dict], flat_text: str) -> list[dict]:
        """Enrich truncated item descriptions using the flat PDF text.

        Parses the flat text to find item blocks (item#, qty, description
        lines, unit_price, total) and matches them to layout items by
        amount to get full multi-line descriptions.
        """
        lines = [l.strip() for l in flat_text.split("\n")]
        # Build a lookup of full descriptions keyed by (amount_str)
        # Parse flat text: detect runs of numeric-line, numeric-line,
        # text-lines, numeric-line, numeric-line
        ...
        while i < len(lines):
            ...
            if not line:
                i += 1
                continue
            # Detect pattern: item_number (small int), quantity, desc..., price, amount
            if _is_num(line) and int(float(line.replace(",", ""))) <= 999:
                ...
                qty = lines[j].strip()
                j += 1
                # Collect description lines (non-numeric)
                desc_parts = []
                while j < len(lines):
                ...
                if not desc_parts:
                    i += 1
                    continue
                # Next numeric lines are unit_price and amount
                unit_price = ""
                amount = ""
        ...
        # Build lookup by normalized amount for matching
        ...
        # Also build a sequential lookup in case amounts are duplicated
        ...
        for idx, item in enumerate(items):
            ...
            desc = item.get("item", "").strip()
            new_desc = None
            # Try sequential match first (most reliable for multi-page docs)
            if idx < len(seq_descs):
                candidate = seq_descs[idx]
                # Verify the layout desc is a substring of the flat desc
...
def _detect_region_quote(address: str) -> str:
    ...
    if region_match:
        return region_match.group(1).title()
    # US state abbreviations (e.g., "Tampa, FL 33609")
    state_match = re.search(r"\b([A-Z]{2})\s+\d{5}\b", normalized)
    if state_match:
...
class ExcelParser:
    """Reads all cell values from an Excel workbook and returns them as text."""
    ...
    def extract_cells(file_path: str) -> dict[str, Any]:
        """Extract structured cell data from the first sheet."""
        wb = openpyxl.load_workbook(file_path, data_only=True)
...
class ExcelDocumentExtractor:
    """Extracts structured data from Excel files by scanning cell content."""
    ...
    def _build_index(self):
        """Build a map of all cells for quick lookup."""
    ...
    def _find_cell_by_label(self, *labels: str) -> Any:
        """Find a cell value to the right of or below a label cell."""
    ...
    def _find_cell_exact_label(self, *labels: str) -> Any:
        """Find a cell value to the right of a label that matches exactly."""
        for (r, c), val in self.cell_map.items():
            val_str = str(val).strip().lower()
            for label in labels:
    ...
    def _collect_address_block(self, start_row: int, col: int, max_lines: int = 6) -> str:
        """Collect consecutive cell values below a starting row in a given column."""
        parts = []
    ...
    def _find_label_row(self, *labels: str) -> tuple[int, int]:
        """Find the row and column of a label cell."""
        for (r, c), val in self.cell_map.items():
            val_str = str(val).strip().lower()
            for label in labels:
                if label.lower() in val_str:
    ...
    def _extract_line_items(self) -> list[dict[str, str]]:
        """Find the line items table by locating the header row."""
        header_row = 0
        ...
        for r in range(header_row + 1, self.ws.max_row + 1):
            ...
            # Check all cells in this row for summary/footer keywords
            row_texts = []
            ...
            # If no quantity and it looks like a summary row, stop
            ...
            # Skip rows where quantity is not numeric (notes, footers)
    ...
    def _detect_currency(self) -> str:
        """Detect currency from cell content."""
    ...
    def _find_monetary_value(self, *labels: str) -> str:
        """Find a monetary value next to a label, checking right then below."""
        for (r, c), val in self.cell_map.items():
            val_str = str(val).strip().lower()
            for label in labels:
                if label.lower() in val_str:
    ...
    def _find_monetary_value_exact(self, *labels: str) -> str:
        """Find monetary value where the label fully matches the cell text."""
        for (r, c), val in self.cell_map.items():
            val_str = str(val).strip().lower()
            for label in labels:
    ...
    def _find_monetary_value_exclude(
        ...
        """Find monetary value matching include keywords but not exclude keywords."""
        for (r, c), val in self.cell_map.items():
            val_str = str(val).strip().lower()
    ...
    def _find_last_monetary_value(self, label: str) -> str:
        """Find the last occurrence of a label with a numeric value to its right."""
        result = ""
        for (r, c), val in sorted(self.cell_map.items()):
            val_str = str(val).strip().lower()
            if label.lower() in val_str:
    ...
    def extract_quote(self) -> dict[str, Any]:
        """Extract quote-specific fields from Excel."""
        ...
        # Try to find supplier from top-right area (company header) or
        # from "Invoice Address" block, or from any cell containing "Ltd/Inc/Corp"
        supplier_name = ""
        supplier_address = ""

        # Strategy 1: Look for company name in top rows (rows 1-3)
        for r in range(1, 4):
            for c in range(1, self.ws.max_column + 1):
                ...
                if val and re.search(r"\b(Ltd|Limited|Inc|Corp|LLC|PLC|GmbH|LLP)\b",
                    ...
                    # Split on comma or newline to get name vs address
        ...
        # Strategy 2: Look under "Invoice Address" label for buyer company (not supplier)
        buyer_company = ""
        for (r, c), val in self.cell_map.items():
            val_str = str(val).strip().lower()
        ...
        if not total:
            # Find the last "total" row that has a numeric value
    ...
    def extract_invoice(self) -> dict[str, Any]:
        """Extract invoice-specific fields from Excel."""
    ...
    def extract_purchase_order(self) -> dict[str, Any]:
        """Extract purchase order-specific fields from Excel."""
        po_number = str(self._find_cell_by_label(
...
def run_data_extraction(file_path: str):
    ...
    if doc_type == "invoice":
        if is_excel:
            result = ExcelDocumentExtractor(file_path).extract_invoice()
        else:
        ...
        if is_excel:
            result = ExcelDocumentExtractor(file_path).extract_quote()
        else:
        ...
        if is_excel:
            result = ExcelDocumentExtractor(file_path).extract_purchase_order()
        else:
...
def main() -> None:
    ...
    for file_path in files:
        ...
        try:
            ...
            if doc_type == "invoice":
                if is_excel:
                    excel_ext = ExcelDocumentExtractor(file_path)
                    result = excel_ext.extract_invoice()
                else:
                ...
                if is_excel:
                    excel_ext = ExcelDocumentExtractor(file_path)
                    result = excel_ext.extract_quote()
                else:
                ...
                if is_excel:
                    excel_ext = ExcelDocumentExtractor(file_path)
                    result = excel_ext.extract_purchase_order()
                else:
