...
from services import email_dispatch_service as _eds_module
...
def _fake_record_workflow_dispatch(**kwargs):
    _tracking_dispatches.append(kwargs)


def _fake_load_workflow_rows(*, workflow_id):
    rows = []
    for d in _tracking_dispatches:
        if d.get("workflow_id") == workflow_id:
            rows.append(
                workflow_email_tracking_repo.WorkflowDispatchRow(
                    ...
                    thread_headers=d.get("thread_headers"),
                )
            )
    return rows
