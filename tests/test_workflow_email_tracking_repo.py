...
from services import db as _db_module


@pytest.fixture(autouse=True)
def _fix_fake_cursor_columns(monkeypatch):
    """Patch the fake DB INSERT column list to include recipient_emails."""
    _orig_dispatch = _db_module._FakeCursor._dispatch

    def _patched_dispatch(self, statement, params):
        ...
        if upper_stmt.startswith("INSERT INTO PROC.WORKFLOW_EMAIL_TRACKING"):
            ...
            return
        return _orig_dispatch(self, statement, params)

    monkeypatch.setattr(_db_module._FakeCursor, "_dispatch", _patched_dispatch)
