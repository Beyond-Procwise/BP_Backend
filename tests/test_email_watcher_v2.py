...
def test_imap_email_fetcher_reads_unseen_emails(monkeypatch):
    class _FakeIMAP:
        def __init__(self) -> None:
        ...
        def search(self, charset, *criteria):
            self.search_calls.append(tuple(criteria))
            return "OK", [b"1 2 3"]

        def fetch(self, msg_id, parts):
            self.fetch_calls.append(msg_id)
            message = EmailMessage()
            message["Message-ID"] = f"<msg-{msg_id}>"
            message["Subject"] = f"Test {msg_id}"
            message["From"] = "test@example.com"
            message["Date"] = f"Thu, 1 Feb 2024 12:00:00 +0000"
            message.set_content(f"Body {msg_id}")
    ...
    from services.email_watcher import ImapEmailFetcher
    ...
    responses = fetcher._fetch_sync(since)

    assert len(responses) >= 1
    assert fake_client.search_calls


def test_register_sent_email_includes_dispatch_metadata(monkeypatch):
