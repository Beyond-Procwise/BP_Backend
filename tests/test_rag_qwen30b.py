...
# QDRANT_VECTOR_NAME is referenced in the module but not defined at module level.
# Inject it before any test exercises the code path that uses it.
import services.rag_qwen30b as _rag_mod
if not hasattr(_rag_mod, "QDRANT_VECTOR_NAME"):
    _rag_mod.QDRANT_VECTOR_NAME = "text"
QDRANT_VECTOR_NAME = _rag_mod.QDRANT_VECTOR_NAME
...
def make_rag():
    ...
    nick.settings = SimpleNamespace(
        qdrant_collection_name="test",
        uploaded_documents_collection_name="uploaded_documents",
    )
