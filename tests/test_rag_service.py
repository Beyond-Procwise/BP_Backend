...
def test_chunk_text_fallback_splits_long_documents(monkeypatch):
    ...
    long_text = "\n\n".join(
        f"Paragraph {i} explains procurement policies and approval workflows."
        for i in range(40)
    )
...
def test_pipeline_answer_returns_documents(monkeypatch):
    ...
    assert "<article class=\"llm-answer__content\">" in result["answer"]
    assert "<h2 class=\"llm-answer__title\">Answer</h2>" not in result["answer"]
...
def test_pipeline_returns_fallback_when_no_retrieval(monkeypatch):
    ...
    assert "<article class=\"llm-answer__content\">" in answer_html
    assert "<h2 class=\"llm-answer__title\">Answer</h2>" not in answer_html
    assert (
...
def test_feedback_acknowledgment_skips_question_preamble(monkeypatch):
    ...
    # The ack message may have its apostrophes HTML-encoded
