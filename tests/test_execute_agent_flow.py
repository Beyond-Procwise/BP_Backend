...
def test_load_prompts_uses_fixture_when_catalog_empty():
    ...
    # The bundled fixture file uses ``template_id`` rather than ``promptId``,
    # so the fallback loader may yield an empty dictionary.  The important
    # behaviour under test is that ``_load_prompts`` does not raise and
...
def test_load_prompts_recovers_when_engine_errors():
    ...
    # Same as above: the fallback fixture file may not match the expected
    # ``promptId`` key, so we only verify resilience – no exception raised
    # and the return value is a valid dict.
    assert isinstance(prompts, dict)
